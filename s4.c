...
static void flush_buf(stats_t *st){
    ...
    fwrite(st->buf,1,st->blen,out_fp); fflush(out_fp);
...
static int extend4(const u64 *v, const u32 rt[4][4], u128 *out, int maxout, stats_t *st){
    ...
    u64 divs[16384]; int nd = gen_divisors(&fbest, divs, 16384);
    if(nd<0){ fprintf(stderr,"too many divisors\n"); return 0; }
...
static void process_chunk(u64 S0, u64 S1, work_t *w, stats_t *st){
    ...
    for(u64 p=1;p<=pmax;p++){
        ...
        u64 S=p*p+q*q;
        while(S<S1){ w->cnt[S-S0]++; S+=2*q+1; q++; }
        w->qnext[p]=(u32)q;
    }
    /* pass 2: offsets */
    u64 T=0; for(u64 i=0;i<n;i++){ w->off[i]=(u32)T; T+=w->cnt[i]; }
    st->pairs += T;
...
int main(int argc, char **argv){
    ...
    for(int t=0;t<nthreads;t++){
        ...
        pthread_create(&th[t],NULL,worker,&st[t]);
    }
    double t0=now_sec();
